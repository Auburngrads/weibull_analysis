...
gif_dir = file.path(root,"images","gifs","surv_pdf")
...
g1 = ggplot(.sd, aes(x = obs, y = vals)) +
  ...
  xlab("Time") + ylab("f(t)") +
  ggtitle(glue::glue("% shaded area under f(t) = {.sp[obs <= limit,min(1-vals)]}"))
...
g2 = ggplot(.sp[obs <= limit,], aes(x = obs, y = 1-vals)) +
  ...
  xlab("Time") + ylab("S(t)") +
  ggtitle(glue::glue("S({limit}) = {.sp[obs <= limit,min(1-vals)]}"))
...
list.files(path = gif_dir, pattern = "*.png", full.names = T) %>%
  ...
  image_write("surv_pdf.gif") # write to current dir
